...
/*       4. Crio o grafo em formato sgb                             */
/*       5. fgets ao invés de gets                                  */
/********************************************************************/
int parse( n_ad, m_ad, nodes_ad, arcs_ad, 
           source_ad, node_min_ad, problem_name, maxlen_ad , g)
...
char    tmp,                     /* temporary */
        in_line[MAXLINE],       /* for reading input line */
  ...
  switch (in_line[0])
    {
      ...
      case 'n':		         /* source(s) description */
		...
		k = sscanf ( in_line,"%c %ld", &tmp, &source);
...
/* --------------------------------------------------------- */
/*  Cria o grafo no formato sgb                              */
*g = gb_new_graph(*n_ad);
if ( (*g == NULL) || ((*g)->n <= 1) ) {
  printf("Não conseguiu converter o grafo do formato DIMACS para o sgb\n"); 
  exit(0);
}
/*printf ("\nordered arcs:\n");*/
for ( u = *nodes_ad; u < *nodes_ad + *n_ad; u++ )
{ 
  i = (u-*nodes_ad)+node_min;
  for ( ta=u -> first; ta != (u+1)-> first; ta++ ){
    /*printf ( " %2ld %2ld %4ld\n",
	     i, ((ta->head)-*nodes_ad)+node_min, ta->len
	     );*/
    gb_new_edge((*g)->vertices+(i-1),
	       (*g)->vertices+(((ta->head)-*nodes_ad)+node_min -1),
	       ta->len);
  }
