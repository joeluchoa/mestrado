...
\citet{zang:80} e \citet{jaffe:84} mostraram que o \rcsp~é 
$\mathcal{NP}$-difícil, mesmo em grafos acíclicos, com restrições sobre 
um único recurso, e com todos os consumos de recursos positivos 
\citep{dumitrescu:03}.  \citet{garey:79} apresentaram uma redução do 
problema da partição para o \rcsp, enquanto \citet{zang:80} reduziram o 
problema da mochila para o nosso problema. 

\citet{hassin:92} mostrou que o \srcsp~ tem solução polinomial se os 
custos dos arcos e consumos de recursos são limitados.
%Temos por \citet{beasley:89} que a solução do \rcsp~é garantidamente 
%elementar (não passa por um mesmo vértice duas vezes) se os custos dos 
%arcos são não-negativos e temos apenas restrições que limitam a 
%quantidade máxima de recursos consumidos ou o grafo é acíclico.
