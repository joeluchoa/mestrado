...
se utiliza de uma relaxação de um problema de programação linear que 
modela o \rcsp com um único recurso. 

Inicialmente, vamos apresentar uma formulação para o problema \rcsp\ 
usando programação linear. Nela teremos uma variável $x_{uv}$ para cada 
...
satisfazem as equações $(1)$ e $(3)$, ou seja, vetores $x$ que contêm um 
caminho de $\scor$ a $\tcor$. Vamos definir também a seguinte função.
...
passo para resolver o \rcsp.  Tendo em vista que o problema é 
...
poderíamos considerar usar um algoritmo de $k$-ésimo menor caminho ({\it k-shortest path}) a partir do 
...
relação a função lagrangiana $L(u^*,x)$ (o que é equivalente a  
