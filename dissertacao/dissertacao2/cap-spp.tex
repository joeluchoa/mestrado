...
menor custo em um grafo direcionado.  A importância do \spp~se deve por 
...
Podemos definir uma \defi{função custo} $c$ em um grafo $G = (V,A)$ como 
sendo uma função sobre $A$, onde, para todo $uv \in A$, $c(uv)$ é o 
valor de $c$ em $uv$ (o custo do arco $uv$). 
...
$\tcor$ o inicio e o fim de $P$ respectivamente, vale que $c(P) \leq c(Q)$ 
...
Vamos definir o seguinte programa linear, que chamamos de primal, e é 
uma relaxação do problema do caminho mínimo: encontrar um vetor $x$ 
indexado por $A$ que
\begin{linearprogram}
  \mbox{minimize}
...
uma solução viável do problema primal. Vamos definir agora, o respectivo 
problema dual, que consiste em encontrar um vetor $y$ indexado por $V$ 
que
\begin{linearprogram}
...
encontrados, e o certificado de não acessibilidade de alguns vértices 
por $\scor$.
...
\begin{lema}{lema da dualidade}
  ...
  Para todo caminho $P$ com início em $\scor$ e término em $\tcor$ e todo $c$-potencial $y$ 
  vale que 
  \[
...
Do lema~\ref{lema:dualidade} tem-se imediatamente os seguintes 
corolários.
...
\begin{corolario}{condição de otimalidade}%
  ...
  Se $P$ é um caminho de $\scor$ a $\tcor$ e $y$ é um $c$-potencial tais que 
  $y(\tcor) - y(\scor) = c(P)$, então $P$ é um 
...
que $P$ é um  é um \textbf{caminho determinado por $\pred$} se
...
\begin{figure}[!ht]
  \begin{center}
    ...
    predecessores induzido por ela.] {\label{fig:pred} Exemplo de uma 
      função predecessor $\pred$ e de uma grafo de predecessores 
      induzido por ela. Acima temos os valores de $\pred$ para cada 
      vértice. O grafo da esquerda mostra as arestas induzidas por 
      $\pred$ em vermelho. A grafo da direita é o grafos de predecessores 
    a partir de $\pred$.}
  \end{center}
\end{figure}
...
função potencial.  O custo dos caminhos que tem como origem o vértice 
...
vértice inicial $\scor$ para um nó final $\tcor$ ou de $\scor$ para todos os outros 
vértices. Apresentamos com detalhes o algoritmo de Dijkstra.
...
grafos onde a função custo possui apenas custos não negativos, ou seja, 
$c(uv) \geq 0$ para todo $uv \in A$. Nosso texto segue de perto 
\citet{fabio:09} e \citet{pf:fluxos}.
...
\cap Q = \emptyset$ e $S \cup Q = V$). O algoritmo conhece caminhos 
partindo de $\scor$ a cada vértice em $S$, caminhos estes que são 
garantidamente de custo mínimo, e conhece caminhos a uma parte dos 
vértices em $Q$. 
%TODO
Cada iteração consiste em retirar um determinado vértice de $Q$, 
...
melhores que os já conhecidos.

O algoritmo recebe um grafo $G = (V,A)$, uma função custo $c$ de $A$ em 
$\NonnegInt$ e um vértice~$\scor$ e devolve uma função-predecessor $\pred$ e 
uma função-potencial~$y$ que respeita~$c$ tais que, para cada vértice 
$\tcor$, se $\tcor$ é acessível a partir de $\scor$, então $\pred$ determina um 
caminho de $\scor$ a $\tcor$ que tem comprimento $y(\tcor) - y(\scor)$, caso contrário 
$y(\tcor)-y(\scor) = nC+1$, onde $C := \max\{ c(uv) \tq uv \in A\}$ \footnote{
  ...
  $y(\tcor) - y(\scor) = nC + 1$, então não existe caminho de $\scor$ a $\tcor$ (condição 
de inacessibilidade, corolário~\ref{corolario:inacessabilidade}).}.
...
\begin{algoritmo}
  ...
  5\x $Q \larr V$  \quad {$\rhd$  $Q$ func. como uma fila com
  prioridades} 
...
\begin{algoritmo}
  ...
  \d5\x $Q \larr V$  \quad {$\rhd$  $Q$ func. como uma fila com
  prioridades} 
...
Na figura~(1) temos o grafo $(V,A)$ com custo sobre os arcos.  Nas 
figuras os vértices em $Q$ têm interior azul claro. A função-potencial 
$y$ é indicada pelos números em azul próximos cada vértice.  Na 
figura~(2) temos que todos os vértices foram inseridos em $Q$ e os 
potencias foram inicializados ($y(\scor)=0$ e o potencial dos demais 
vértices é $n \times C + 1 = 6 \times 7 + 1 = 43$).

Um vértice sendo examinado tem a cor rosa. O arco sendo examinado tem a 
cor azul (na figura~(3) o vértice sendo examinado é $\scor$ e o arco sendo 
examinado é $sv$).

Os vértices em $S$ (já examinados) têm a cor verde. Na figura~(10) vemos 
...
\begin{teorema}{da corretude}
  ...
  Dado um grafo $(V,A)$, uma função custo~$c$ e um vértice $\scor$ o 
  algoritmo \Dijkstra\ corretamente encontra um caminho de custo mínimo 
  de $\scor$ a~$\tcor$, para todo vértice $\tcor$ acessível a partir de $\scor$.  
\end{teorema}
...
no pior caso):
\begin{enumerate}
  \item escolher um vértice com potencial mínimo, que pode gastar tempo 
    $O(n)$ e é executada até $n$ vezes (até $Q$ ficar vazio), ou seja, 
    consome tempo $O(n^{2})$;
  \item e atualizar o potencial, que pode acontecer para todas as 
    arestas, ou seja, gasta tempo $O(m)$.
\end{enumerate}
...
\citep{FredTarjan:Fibonacci}, que permitem diminuir o tempo gasto para 
encontrar um vértice com potencial mínimo, gerando assim implementações 
que consomem menos tempo para resolver o problema.
...
Uma \defi{fila de prioridades}~\cite{ahuja:netflows, clr:introalg-1999} 
...
O consumo de tempo  do algoritmo Dijkstra pode variar conforme a 
...
A tabela a seguir resume o consumo de tempo de várias implementações de
...
Fredman e Tarjan~\cite{FredTarjan:Fibonacci} observaram que como o 
algoritmo de Dijkstra examina os vértices em ordem de distância a partir 
de $\scor$, o algoritmo está, implicitamente, ordenando estes valores.  
Assim, qualquer implementação do algoritmo de Dijkstra realiza, no pior 
caso, $\Omega(n \log n)$ comparações e faz $\Omega(m + n \log n)$ 
...
potencias as vezes que for necessário, até que todos os potencias 
satisfaçam a condição de otimalidade. Neste algoritmo, os potenciais dos 
vértices têm o mesmo significado dos potencias utilizados no algoritmo 
\Dijkstra, só que agora os potenciais só se tornam definitivos após 
terminar a execução do algoritmo.

O algoritmo recebe um grafo $G = (V,A)$, uma função custo $c$ de $A$ em 
$\Int$ e um vértice~$\scor$ e devolve uma função-predecessor $\pred$ e uma 
função-potencial~$y$ que respeita~$c$ tais que, para cada vértice $\tcor$, 
se $\tcor$ é acessível a partir de $\scor$, então $\pred$ determina um caminho 
de $\scor$ a $\tcor$ que tem comprimento $y(\tcor) - y(\scor)$, caso contrário 
$y(\tcor)-y(\scor) = nC+1$, onde $C := \max\{ c(uv) \tq uv \in A\}$.
Versão genérica do algoritmo.

\begin{algoritmo}
  \Ford{} $(V, A, c, \scor)$ \quad {$\rhd$  $(V, A, c)$ não possui ciclos 
  negativos}
...
O consumo de tempo desta versão é $O(n^2mC)$ \citep{pf:fluxos}. O 
consumo de tempo é tão elevado porque o algoritmo pode examinar cada 
arco muitas veze (o valor de $y(v)$ pode diminuir muitas vezes antes de 
atingir seu valor final). A seguir temos a melhor versão conhecida, do 
ponto de visa do consumo assintótico de tempo, para o problema do 
caminho mínimo com custos arbitrários.
...
caso, pode-se dizer que o algoritmo é $O(n^3)$.
